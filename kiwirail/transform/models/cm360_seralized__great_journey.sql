{{ config(
    materialized='table',
    schema=env_var('CM360_SUFFIX', 'cm360_transformed__great_journey'),
) }}
WITH cm360reference AS (
    ...
    FROM {{ source('cm360_raw', 'cm360_report_stream') }}
    WHERE LOWER(JSON_VALUE(JSON_EXTRACT(data, "$.advertiser"))) = 'gjnz'
)
