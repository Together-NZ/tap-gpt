...
with dash_table AS (
SELECT * FROM `wcet-main.google_ads_search_transformed__beervana.google_ads_search__beervana`
)
SELECT *,
...
CASE WHEN 'INTENT' IN (select distinct funnel from `wcet-main.dash_table__beervana.dash_table__beervana`) and lower(publisher) != 'demand gen' then 'INTENT'
