...
WITH ranked_data AS (
    ...
    FROM
        `bepure-main.facebook_raw__bepure.ads_insights`
),
...
ad_data AS (
    ...
    FROM `bepure-main.facebook_raw__bepure.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `bepure-main.facebook_raw__bepure.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `bepure-main.facebook_raw__bepure.campaigns`
),
