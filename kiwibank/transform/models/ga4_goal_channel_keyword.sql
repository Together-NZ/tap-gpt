...
WITH channels as (
  SELECT *,
    ...
    CASE 
        ...
        WHEN TRIM(LOWER(site_name)) = 'google' AND lower(sessionSourceMedium) LIKE '%cpc%' THEN 'Paid Search'
...
funnel_campaign AS (
     select distinct funnel, campaign_name from `kiwibank-main.dash_table__ao_social_boosting.dash_union__ao_social_boosting`
     UNION ALL
     select distinct funnel, campaign_name from `kiwibank-main.dash_table__business_banking.dash_union__business_banking`
     UNION ALL
     select distinct funnel, campaign_name from `kiwibank-main.dash_table__credit_card.dash_union__credit_card`
     UNION ALL
     select distinct funnel, campaign_name from `kiwibank-main.dash_table__everyday_banking_join_kiwibank.dash_union__everyday_banking_join_kiwibank`
     UNION ALL
     select distinct funnel, campaign_name from `kiwibank-main.dash_table__everyday_banking_retail_deposit.dash_union__everyday_banking_retail_deposit`
     UNION ALL
...
dash AS (
     ...
     FROM `kiwibank-main.dash_table__ao_social_boosting.dash_union__ao_social_boosting`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
          REGEXP_REPLACE(TRIM(creative_descr), r'[_\-\s]+', '') AS creative_name,
          ROW_NUMBER() OVER(PARTITION BY LOWER(creative_descr)) AS row_num
     FROM `kiwibank-main.dash_table__business_banking.dash_union__business_banking`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
          REGEXP_REPLACE(TRIM(creative_descr), r'[_\-\s]+', '') AS creative_name,
          ROW_NUMBER() OVER(PARTITION BY LOWER(creative_descr)) AS row_num
     FROM `kiwibank-main.dash_table__credit_card.dash_union__credit_card`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
          REGEXP_REPLACE(TRIM(creative_descr), r'[_\-\s]+', '') AS creative_name,
          ROW_NUMBER() OVER(PARTITION BY LOWER(creative_descr)) AS row_num
     FROM `kiwibank-main.dash_table__everyday_banking_join_kiwibank.dash_union__everyday_banking_join_kiwibank`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
          REGEXP_REPLACE(TRIM(creative_descr), r'[_\-\s]+', '') AS creative_name,
          ROW_NUMBER() OVER(PARTITION BY LOWER(creative_descr)) AS row_num
     FROM `kiwibank-main.dash_table__everyday_banking_retail_deposit.dash_union__everyday_banking_retail_deposit`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
          REGEXP_REPLACE(TRIM(creative_descr), r'[_\-\s]+', '') AS creative_name,
          ROW_NUMBER() OVER(PARTITION BY LOWER(creative_descr)) AS row_num
     FROM `kiwibank-main.dash_table__fraud_and_scams.dash_union__fraud_and_scams`
     UNION ALL
     SELECT DISTINCT 
          TRIM(creative_descr) AS old_creative,
          --campaign_name AS platform_campaigns,
...
CASE
  ...
  WHEN (LOWER(COALESCE(md.campaign_name,''))    LIKE '%search%' AND
       LOWER(COALESCE(sessionSourceMedium,'')) LIKE '%cpc%' ) OR sessionCampaignName='RLNZ005 - Travel Feed - Compare - NZ'
  THEN 'SEARCH'
  ELSE 'OTHER'
END AS media_format,
