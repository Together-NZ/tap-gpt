...
with models.DAG(
    ...
    def set_env_vars_ga4(id,name,goal):
        env = get_meltano_env()
        if goal == "session":
            ...
            env["GA4_GOAL"] = "goal"
        env["BQ_DATASET"] = f"ga4_raw__{name}"
        env["BQ_METHOD"] = "gcs_stage"
        env["DBT_BIGQUERY_METHOD"] = 'oauth'
        env["DBT_BIGQUERY_PROJECT"] = 'moe-main'
        env["DBT_BIGQUERY_AUTH_METHOD"]='oauth'
        env["DBT_BIGQUERY_DATASET"] = f'ga4_transformed__{name}'       
    ...
    for id,name in ga4_list.items():
        for goal in goal_list:
            kube_ga4 = KubernetesPodOperator(
                name="moe-ga4-to-bigquery-"+name+"-"+goal,
                task_id="moe-ga4_to_bigquery_"+name+"-"+goal,
                namespace="composer-user-workloads",
                image=IMAGE,
                arguments=["--environment=prod", "run", "tap-ga4", "target-bigquery",f"dbt-bigquery:ga4_{name}_{goal}_models"],
                container_resources=k8s_models.V1ResourceRequirements(
                    limits={"memory": "1000M", "cpu": "500m"},
                ),
                env_vars=set_env_vars_ga4(id,name,goal)
            )
    ...
    for task in ga4_task_list:
        kube_dash_union >> task >> kube_ga4_merge >> [kube_ga4_domestic,kube_ga4_international]
